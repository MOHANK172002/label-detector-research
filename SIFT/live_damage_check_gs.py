...
DEFAULT_FPS       = 30
DEFAULT_FORMAT    = "MJPG"
GS_CAMERA_NAME    = "Global Shutter Camera"   # v4l2 device name to auto-detect
...
def gstreamer_pipeline(cam_index=DEFAULT_CAM_INDEX, width=DEFAULT_WIDTH,
    ...
    """
    Build a GStreamer pipeline for v4l2src (see gstream.py).
...
def main():
    ...
    parser.add_argument("--camera", type=int, default=None,
                        help=f"v4l2 device index /dev/videoN "
                             f"(default: auto-detect '{GS_CAMERA_NAME}')")
    ...
    parser.add_argument("--format", default=DEFAULT_FORMAT,
                        help=f"Capture format MJPG/YUYV (default {DEFAULT_FORMAT})")
    ...
    cam_index = args.camera if args.camera is not None else find_camera_index()
    print(f"Camera index : {cam_index} (/dev/video{cam_index})")
